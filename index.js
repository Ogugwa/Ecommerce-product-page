...
const desktopLink = document.querySelector(".nav-links");
const navBar = document.querySelector("nav");
if (!desktopLink || !navBar)
    throw new Error("Navbar or nav-links missing");
// Hamburger setup
...
function toggleSidebar() {
    if (!desktopLink)
        return;
    isOpen = !isOpen;
    hamburgerMenu.innerHTML = `<img src="${isOpen ? hamburgerImages[1] : hamburgerImages[0]}" alt="${isOpen ? "Close" : "Menu"} Icon" />`;
...
function appendCloseIcon() {
    if (window.innerWidth > 768)
        return; // Only add close icon in mobile view
    if (!desktopLink)
        return;
    // Prevent duplicates
    if (desktopLink.querySelector(".close-icon"))
        return;
...
desktopLink.querySelectorAll("a").forEach((link) => link.addEventListener("click", () => {
    isOpen = false;
    desktopLink.style.transform = "translateX(-100%)";
    hamburgerMenu.innerHTML = `<img src="${hamburgerImages[0]}" alt="Menu Icon" />`;
}));
